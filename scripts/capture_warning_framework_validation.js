...
const artifactDir = process.env.ARTIFACT_DIR
  ...
  : path.join(repoRoot, 'qa_artifacts', 'warning_framework_2026_06_22');
fs.mkdirSync(artifactDir, { recursive: true });
const baseUrl = (process.env.BASE_URL || 'http://127.0.0.1:8790').replace(/\/$/, '');
const pageUrl = baseUrl + '/index.html?warning_framework_check=' + Date.now();
...
async function main() {
  ...
  await page.waitForFunction(() => typeof window.addCptDirectly === 'function' && typeof window.setCaseBuilderUserModifier === 'function');
  ...
  const validation = {
    ...
    pageUrl,
    consoleMessages,
    cases: { ncci, separate, doc, mobile },
